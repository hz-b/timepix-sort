...
from .config import TDCEventType
from .data_model import Event, PixelEvent, PixelPosition, TimeOfFlightEvent
...
def process_header(datum: int):
    name = "".join([chr((datum >> 8 * i) & 0xFF) for i in range(4)])
    if name != "TPX3":
...
def chunks(stream):
    while len(stream) > 0:
        chip_nr, n_entries = process_header(int(stream[0]))
        event = stream[1: n_entries + 1]
        yield Event(chip_nr=chip_nr, event=event)
        stream = stream[n_entries + 1:]
    return
...
def process_chunks(events: Sequence[Event], trigger_mode, tot_min):
    for ev in events:
        for idx, datum in enumerate(ev.event):
